...
# 1. Page Configuration & Custom Professional Style
...
st.markdown(f"""
<style>
/* Natural Background Setup (Contain) */
...
/* High Contrast English Typography */
...
# Data Initialization with Column Cleanup
if 'balance_data' not in st.session_state:
    try:
        ...
        # Enforce column structure for points logic
...
try:
    ...
    # Updated Title Requirement: ELA Teachers' Schedule
    st.subheader(f"📅 ELA Teachers' Schedule - {selected_day}")
    ...
    sel_sess = st.sidebar.selectbox("Select Required Period:", session_cols)

    # Substitution Filtering Logic
    available = []
    ...
    st.subheader(f"🔍 Recommended Substitutes for {sel_sess}")
    col_sel, col_shu = st.columns([3, 1])
    with col_shu: 
        if st.button("🔀 Shuffle Substitutes"): random.shuffle(available)
    with col_sel:
        sub_t = st.selectbox("Choose Candidate:", available) if available else st.warning("No matches found")
    ...
    if sub_t and st.button("✅ Confirm Substitution & Sync"):
        ...
        try:
            ...
            st.success("✅ Google Sheets Synced Successfully!")
        except:
            st.warning("⚠️ Sync failed locally. Check Cloud Secrets.")
        st.balloons()

    # 4. Net Balance Performance Overview
    st.divider()
    st.subheader("📈 Teachers' Performance (Net Balance)")
    res_df = st.session_state.balance_data.copy()
    res_df['Net'] = res_df['Credit'] - res_df['Debit']
    
    # Financial Display Style
